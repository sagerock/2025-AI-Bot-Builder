...
def create_api_key(api_key_data: APIKeyCreate, db: Session = Depends(get_db)):
...
def get_api_keys(provider: Optional[str] = None, db: Session = Depends(get_db)):
...
def update_api_key(key_id: str, api_key_data: APIKeyUpdate, db: Session = Depends(get_db)):
    """Update an API key"""
