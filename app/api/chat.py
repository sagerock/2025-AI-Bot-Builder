...
@router.post("/{bot_id}/json", response_model=ChatResponse)
async def chat_json(
    bot_id: str,
    request: ChatRequest,
    db: Session = Depends(get_db)
    ...
    """
    JSON API endpoint for sending messages to a bot (for Make.com, Zapier, etc.)

    This endpoint accepts JSON body instead of form data, making it easier
    to integrate with automation platforms.
    ...
    # Fire webhook for conversation_started if this is a new conversation
    is_new_conversation = not request.session_id or conversation.id == session_id
    if is_new_conversation:
        WebhookService.trigger_event(
            db=db,
            event="conversation_started",
    ...
    # Save user message
    MemoryService.add_message(db, conversation.id, "user", message)
    ...
    WebhookService.trigger_event(
        ...
        user_id=None,
        user_email=None,
        data={"message": message, "has_file": False, "file_name": None}
    )

    # Generate response
    try:
        response_text = ChatService.chat(bot, message, history, rag_contexts, None)
    except Exception as e:
        import traceback
        error_detail = f"AI provider error: {str(e)}\n{traceback.format_exc()}"
